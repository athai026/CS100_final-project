...
> ## Class Diagram
 ...
 > ## Contributions
   ...
   * Arman: I was responsible for a Search class and its corresponding unit tests. From past experience with one of our labs involivng spreadsheets, I was able to utilize  the search_and, search_or, and search_not, however I only implemented the search_not. This fix allows users to increase search possibilities to better narrow down results. I ended up adding the corresponding functions in the main to allow users to utilize the search_not inverter. Since these methods were implemented previously, they all were fairly easy to understand and design. The most challenging aspect was finding an efficient way to implement search_not in the main for the user experience. Since the not function can double, or even triple our options list, I found it better to implement an option after each selection if the user wanted to filter out the said category in the search. So I went in and added an if statement in each switch case to do just that.
